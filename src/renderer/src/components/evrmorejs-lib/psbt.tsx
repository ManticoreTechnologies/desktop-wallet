...
export class Psbt {
  ...
  addInput(inputData: PsbtInputExtended): this {
    //#console.log("ADDING INPUT INSIDE PSBT")
    if (
      arguments.length > 1 ||
      !inputData ||
    ...
    //#console.log("Checking taprootinput fields")
    checkTaprootInputFields(inputData, inputData, 'addInput');
    //#console.log("Checking inputs for partial sigs fields")
    checkInputsForPartialSig(this.data.inputs, 'addInput');
    if (inputData.witnessScript) checkInvalidP2WSH(inputData.witnessScript);
    //#console.log("aaand aosdfnp")
    const c = this.__CACHE;
    this.data.addInput(inputData);
    //#console.log("Check tx input cache")
    const txIn = c.__TX.ins[c.__TX.ins.length - 1];
    checkTxInputCache(c, txIn);
    //#console.log("Finished checking input cache")
    const inputIndex = this.data.inputs.length - 1;
    const input = this.data.inputs[inputIndex];
    //#console.log("Add non witness tx cache")
