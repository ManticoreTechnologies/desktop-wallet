...
function extractTaprootSigs(input: PsbtInput): Buffer[] {
  ...
  if (input.tapScriptSig)
    sigs.push(...input.tapScriptSig.map((s: any) => s.signature));
  if (!sigs.length) {
...
function checkIfTapLeafInTree(
  ...
  if (newInputData.tapMerkleRoot) {
    const newLeafsInTree = (newInputData.tapLeafScript || []).every((l: any) =>
      isTapLeafInTree(l, newInputData.tapMerkleRoot),
    );
    const oldLeafsInTree = (inputData.tapLeafScript || []).every((l: any) =>
      isTapLeafInTree(l, newInputData.tapMerkleRoot),
    );
...
function sortSignatures(input: PsbtInput, tapLeaf: TapLeafScript): Buffer[] {
  ...
  return (input.tapScriptSig || [])
    .filter((tss: any) => tss.leafHash.equals(leafHash))
    .map((tss: any) => addPubkeyPositionInScript(tapLeaf.script, tss))
    .sort((t1: any, t2: any) => t2.positionInScript - t1.positionInScript)
    .map((t: any) => t.signature) as Buffer[];
}
...
function findTapLeafToFinalize(
  ...
  const tapLeaf = (input.tapLeafScript || [])
    .sort((a: any, b: any) => a.controlBlock.length - b.controlBlock.length)
    .find((leaf: any) =>
